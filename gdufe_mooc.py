...
class GdufeMooc:
    ...
    def into_main_page(self):
        self.browser_lib.open_url("https://gdufe.xuetangonline.com")
        ...
        result = self.browser_lib.get_element(
            {
                ...
                "type_name": "ul",
                "text_type": "class",
                "text": "list-inline list-unstyled-radio",
            },
            10,
        )
    ...
    def submit_answer(self):
        ...
        submit = self.browser_lib.get_element(
            {"type": "tag", "type_name": "span", "text_type": "text", "text": "提交.*剩余"},
            3,
        )
        if submit:
            submit.click()
...
class BrowserLib:
    ...
    def get_elements(
        self,
        selector={"type": "tag", "type_name": "div", "text_type": "text", "text": ""},
        _timeout=3,
    ):
        """获取元素列表
        :param selector: 选择器
        ...
        try:
            ...
            for _ in range(999999):
                if selector["type"] == "tag":
                    ...
                    elements = self.browser.find_elements_by_id(selector["type_name"])
                elif selector["type"] == "class":
                    elements = self.browser.find_elements_by_class_name(
                        selector["type_name"]
                    )
                elif selector["type"] == "name":
                    elements = self.browser.find_elements_by_name(selector["type_name"])
                else:
                    return []
                element_list = []
                element_text = None
